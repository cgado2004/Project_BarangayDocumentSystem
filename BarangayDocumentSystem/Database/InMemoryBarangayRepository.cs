...
public class InMemoryBarangayRepository : IBarangayRepository
{
    ...
    public IEnumerable<Resident> SearchResidents(string term)
    {
        ...
        return _residents.Where(r =>
            r.GetFullName().IndexOf(term, StringComparison.OrdinalIgnoreCase) >= 0 ||
            r.GetSortableName().IndexOf(term, StringComparison.OrdinalIgnoreCase) >= 0 ||
            r.Purok.IndexOf(term, StringComparison.OrdinalIgnoreCase) >= 0 ||
            r.ContactNumber.IndexOf(term, StringComparison.OrdinalIgnoreCase) >= 0 ||
            r.Occupation.IndexOf(term, StringComparison.OrdinalIgnoreCase) >= 0);
    }
