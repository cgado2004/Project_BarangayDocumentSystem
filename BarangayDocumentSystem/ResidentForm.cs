...
public partial class ResidentForm : DialogBase
{
    ...
    private readonly ComboBox _gender = UiFactory.ComboBox(Enum.GetNames<Gender>());
    private readonly ComboBox _civil = UiFactory.ComboBox(Enum.GetNames<CivilStatus>());
    ...
    private void BuildFields()
    {
        ...
        void Add(string caption, Control control, int column, int span = 1)
        {
            ...
            if (span > 1) grid.SetColumnSpan(grid.Controls[^1], span);
        }
    ...
    private void Save(object? sender, EventArgs e)
    {
        ...
        Result = new ResidentDetails(
            ...
            _dob.Value.Date,
            Enum.Parse<Gender>(_gender.SelectedItem?.ToString() ?? nameof(Gender.Male)),
            Enum.Parse<CivilStatus>(_civil.SelectedItem?.ToString() ?? nameof(CivilStatus.Single)),
