...
public static class InputValidator
{
                              ...
                              decimal min = 0m, decimal max = 1_000_000m)
    {
        ...
        if (amount < min)
            return Check.Fail($"{field} cannot be less than {BusinessRules.DisplayFormat.Peso(min)}.");
        if (amount > max)
            return Check.Fail($"{field} cannot be more than {BusinessRules.DisplayFormat.Peso(max)}.");
    ...
    public static Check AssessedAmount(decimal value, string field, decimal max = 100_000m)
    {
        ...
        if (value > max)
            return Check.Fail($"{field} is beyond the plausible range (at most {BusinessRules.DisplayFormat.Peso(max)}).");
        return Check.Pass;
    }
