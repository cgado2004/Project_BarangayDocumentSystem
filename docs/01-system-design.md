...
| `MySqlBarangayRepository` | Database | MySQL implementation (load once, write through) |
...
- **Single user.** Data is stored in MySQL but cached in memory; two running copies do not see each other's changes.
...
Storage is `Database/MySqlBarangayRepository.cs`, selected in `Program.cs`.
See the README for setup (`appsettings.json`) and for how saving works.
