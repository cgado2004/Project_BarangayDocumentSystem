...
> **Layout update:** the three-project split described below has since been merged into a single project with `DBContext/`, `Helper/`, `Interfaces/`, `Models/` and `Service/` folders. Paths below are the old ones; see the README for the current layout.
