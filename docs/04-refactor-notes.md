...
> **Layout update:** the three-project split described below has since been merged into a single project (now `Database/`, `Models/`, `Interfaces/`, `BusinessRules/`, `Views/`, `Forms/`, `CustomControls/`, `UIHelpers/`), and `InMemoryBarangayRepository` was replaced by `MySqlBarangayRepository`. The project was then converted from an SDK-style `net8.0-windows` project to a classic .NET Framework 4.8 project (`App.config`, `app.manifest`, `Properties/AssemblyInfo.cs`, old-style `.csproj`), which is why a few .NET 6+-only APIs (`TextBox.PlaceholderText`, `ArgumentNullException.ThrowIfNull`, generic `Enum.Parse<T>`, and one default interface member) were rewritten in their .NET Framework equivalents. Paths below are the old ones; see the README for the current layout.
