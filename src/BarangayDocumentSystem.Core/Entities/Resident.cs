...
/// resident exists perfectly well without any request, and because a request
/// is a historical record. If someone corrects a resident's address next year,
/// that must not quietly rewrite a certificate issued last year.
/// </summary>
public class Resident
{
    ...
    /// <summary>The purok this resident lives in. I offer only the real puroks
    /// of Magugpo Poblacion, so the value cannot be mistyped.</summary>
    ...
    /// <summary>
    /// When the person began residing in the barangay. I use this to run the
    /// six-month residency test RA 11261 requires before I can issue a
    ...
    /// <summary>
    /// RA 11261 may be availed only ONCE in a person's life. I set this when
    /// a first-time jobseeker certificate is released, and I check it before
    /// I ever issue another.
    /// </summary>
    ...
    /// usable middle name.
    ///
    /// I pulled this out of GetFullName and GetSortableName because both had
    /// the same three lines, and both had the same bug: they took
    /// MiddleName[0] without checking it was a letter. A middle name of "."
    /// printed as "Juan .. Dela Cruz" on the certificate, and one starting
    /// with a hyphen printed "Juan -. Dela Cruz".
    ///
    /// Now I look for the first actual LETTER, so punctuation at the start
    /// cannot leak onto an official document.
    /// </summary>
