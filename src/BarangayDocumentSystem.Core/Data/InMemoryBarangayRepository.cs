...
/// setup, and I pre-load it with sample residents so there is something on
/// screen the moment the program starts. The data is lost when the app closes,
/// which is exactly why I also wrote the MySQL version beside it.
/// </summary>
public class InMemoryBarangayRepository : IBarangayRepository
{
        ...
        Resident resident, DocumentType type, string purpose, ClearanceScope scope)
    {
        ...
        var assessment = _feeSchedule.Assess(resident, type, scope);
    ...
    /// <summary>
    /// My sample residents, using the real purok names of Magugpo Poblacion.
    ///
    /// I gave each one a different situation on purpose, so that during the
    /// demo I can show every branch of the fee rules without having to invent
    /// data on the spot in front of the panel.
    /// </summary>
    private void SeedSampleData()
    {
        ...
        // I charge the full ₱200 despite her solo-parent tag, for the reason
        // I gave above.
        CreateRequest(ana, DocumentType.BarangayBusinessClearance,
                      "Sari-sari Store Renewal", ClearanceScope.Local);
