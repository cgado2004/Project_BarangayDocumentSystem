...
public class ResidentsView : ViewBase
{
    ...
    private readonly PillButton _btnDelete = new() { Text = "Delete", Look = PillButton.Style.Outline, Accent = AppTheme.Danger };
    ...
    public ResidentsView(IBarangayRepository repo, FeeSchedule fees)
    {
        ...
        var historyCard = new Card { Dock = DockStyle.Bottom, Height = 190, Margin = new Padding(0, AppTheme.Gap, 0, 0) };
        _historyTitle.Text = "Select a resident to see their requests";
        _historyTitle.Font = AppTheme.Subhead;
        _historyTitle.ForeColor = AppTheme.Ink;
        ...
        Controls.Add(body);
        Controls.Add(PageHeader("Residents", "The barangay registry"));
    }
    ...
    /// the duplication, so now both grids in the app come through this method.
    /// </summary>
    internal static void StyleGrid(DataGridView g)
    {
        g.BackgroundColor = AppTheme.Surface;
        g.BorderStyle = BorderStyle.None;
        g.CellBorderStyle = DataGridViewCellBorderStyle.SingleHorizontal;
        g.GridColor = AppTheme.Border;
        g.EnableHeadersVisualStyles = false;
        g.ColumnHeadersDefaultCellStyle.BackColor = AppTheme.Surface;
        g.ColumnHeadersDefaultCellStyle.ForeColor = AppTheme.Muted;
        g.ColumnHeadersDefaultCellStyle.Font = AppTheme.SmallBold;
        ...
        g.DefaultCellStyle.Font = AppTheme.Body;
        g.DefaultCellStyle.ForeColor = AppTheme.Ink;
        g.DefaultCellStyle.SelectionBackColor = AppTheme.LavenderSoft;
        g.DefaultCellStyle.SelectionForeColor = AppTheme.Ink;
    ...
    private Resident? Selected()
    {
        ...
        return _repo.Residents.FirstOrDefault(r => r.ResidentId == id);
    }
    ...
    private void EditSelected()
    {
        ...
        using var dlg = new ResidentDialog(r);
        if (dlg.ShowDialog(this) != DialogResult.OK || dlg.Result is null) return;
        _repo.UpdateResident(r, dlg.Result);
        LoadGrid();
    }
    ...
    private void NewRequestForSelected()
    {
        ...
        using var dlg = new RequestDialog(r, _fees);
        if (dlg.ShowDialog(this) != DialogResult.OK) return;

        _repo.CreateRequest(r, dlg.SelectedType, dlg.Purpose, dlg.Scope);
