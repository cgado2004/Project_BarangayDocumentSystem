...
4. Open DashboardControl, ResidentsControl, and RequestsControl in the designer
   ...
   The `.Theme.cs` partial files restyle the pages at runtime only, so the
   designers show the plain layout and must still open cleanly.
...
The window opens at 1360×860 in the navy theme: a white sidebar with the
barangay seal, the gold BARANGAY overline, bold Magugpo Poblacion, and the
City of Tagum / Davao del Norte lines at the bottom; a navy hero banner with
the round seal, gold divider, and Punong Barangay line; and six stat cards —
RESIDENTS 7, PENDING 2, READY 1, RELEASED 2, COLLECTED PHP 50.00, ISSUED
FREE 1 — each with its colored left border. Purok pill chips and horizontal
navy bars for requests by document type sit below the cards, and the red
footer line reads 7 residents · 6 requests · PHP 50.00 collected. Residents
and Document requests share the theme: navy table headers, light-blue
selected rows, navy primary buttons, outlined secondary buttons, and the
details panel as a card.
...
historical text, templates, form submissions, multi-page printing, and the
navy theme tokens and helpers (fonts, caching, rounded cards).
